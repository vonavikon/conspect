...
Перед публикацией прочитайте «Известные ограничения» ниже: rate limit в этой сборке общий, не по адресам, и при публичном доступе чужой мусор может выбить лимит вам самим.
...
- **Rate limit общий, не по адресам.** Счётчики глобальные. Пока сервер закрыт (локально или за туннелем) это неважно. На публичном адресе поток чужих запросов выбирает лимит, и отказ получаете вы: ставьте ограничение по IP на уровне reverse-proxy.
