...
describe("REASON_TEXT", () => {
  ...
  // canonical из copy.md: формулировки, которые видит пользователь. Копия в REASON_TEXT
  // с тех пор ушла вперёд copy.md (более конкретные и actionable формулировки — каждая
  // заканчивается понятным следующим шагом), тест сверяется с actual copy, а не с
  // исходным (устаревшим) md — так его снова легко ловить, если копия разъедется опять.
  it("каноничные формулировки из copy.md", () => {
    ...
    expect(REASON_TEXT.not_configured).toBe(
      "Сервер не настроен. Откройте настройки расширения и укажите адрес сервера и токен.",
    );
